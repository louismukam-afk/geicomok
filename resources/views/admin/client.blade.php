...
@section('content')
    ...
    <div class="col-md-12">
        ...
        <td><a href="#multi-delete" id="multi-delete" onclick="sendToDelete('{{route('mdelete_client')}}')"  class="btn btn-danger btn-xs pull-right"><span class="glyphicon glyphicon-trash"></span> remove</a></td>
        ...
        <table class="table table-bordered table-striped table-condensed table-inverse" style="margin-top: 15px;">
            ...
            @foreach($client as $c)
                <tr>
                    <td>{{$i++}}</td>
                    <td><a href="#edit_c" data-toggle="modal" data-backdrop="false" onclick="edit_c({{$c->id}},'{{$c->nom}}','{{$c->telephone}}',{{$c->id_pays}},'{{$c->ville}}','{{$c->email}}','{{$c->adresse}}','{{$c->boite_postale}}')"><strong ><span class="glyphicon glyphicon-edit"></span> {{$c->nom}}</strong></a></td>
                    <td>{{$c->telephone}}</td>
                    <td>
                        @if($c->pays)
                            {{$c->pays->nom}}
